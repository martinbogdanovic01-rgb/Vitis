 ...
 *
 *  v1: original version as used in the MO7 ES2 courses (date unknown)
 *  v2: change PLL clock frequency from 49.152 MHz to 50.000 MHz to match sample clock of ecen427 wrapper (23-4-2026)
 *
 */
...
XIicPs Iic; // XIicPs instance data. I2C is used to configure the audio codec
 ...
 * PLL for a VCO frequency = 50.000 MHz, and an audio sample rate of 48.828125 KHz
 * This sample frequency corresponds to the sample frequency of the I2S bus
 * of the ecen427 wrapper
 * ---------------------------------------------------------------------------- */
void AudioPllConfig() {
	...
	/* 	MCLK = 10 MHz
		R = 0101 = 5, integer PLL operation mode

		PLL required output = 1024x48.828125 KHz
		(PLLout)			= 50.000 MHz

		PLLout/MCLK			= 50.000 MHz/10.000 MHz
							= 5.000
							= R
							= 5 */
	...
	u8TxData[2] = 0x00; // byte 6 - M[15:8](value for M is ignored in integer operation mode, use default value from data sheet)
	u8TxData[3] = 0xFD; // byte 5 - M[7:0] (value for M is ignored in integer operation mode, use default value from data sheet)
	u8TxData[4] = 0x00; // byte 4 - N[15:8](value for N is ignored in integer operation mode, use default value from data sheet)
	u8TxData[5] = 0x0C; // byte 3 - N[7:0] (value for N is ignored in integer operation mode, use default value from data sheet)
	u8TxData[6] = 0x28; // byte 2 - 7 = reserved, bits 6:3 = R[3:0], 2:1 = X[1:0], 0 = PLL operation mode
	u8TxData[7] = 0x01; // byte 1 - 7:2 = reserved, 1 = PLL Lock, 0 = PLL enable
